...
nline=$(cat SAVEINFO | sed -n '/NLINE/{s/NLINE=//;s/ #.*//;p}' ) # get the number behind NLINE which is line number in vim
newdefect='SAVEINFO'
nline=($nline)
...
if [ ${nline[0]} = neighbor ]; then
    ...
    #echo 'line' $nline 'SAVEINFO updated'
else
...
#cat $newdefect
#chmod -R 755 SAVEINFO

# orginal version will omit other arguments in SAVEINFO, like FREYCORR
