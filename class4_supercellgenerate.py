...
class supercellgenerate:
    ...
    def supercopy(self,fromposcarfile='CONTCAR'):
        ...
        for f in list_files:
            ...
            os.system('chmod u+x %s' % (self.folder_to/f))
    ...
    def superposcar(self, orthomatrix=[[1, 1, 0], [1, -1, 0], [0, 0, 1]],superarray=[3, 2, 2]):
        ...
        fname='POSCAR.perfect.vasp'
        struc2poscar(superstruct, self.folder_to, fname=fname, selective_dynamics=False)
    ...
    def superincar(self, isdefect=True):
        ...
        if isdefect: # 'AEXX': self.aexx,
            ...
            change_files.incar_change({'ISIF':2, 'ISPIN':2, 'NSW':200, 'NELM':200, 'LVHAR':'.TRUE.'},
	    		popkey=['LCALCEPS', 'ISTART', 'ICHARG', 'KPAR', 'NELMIN', 'ISYM','MAGMOM'])
        else:
            # for a perfect supercell, turn off the symmetry
            change_files.incar_change({'ISIF':2, 'ISPIN':2, 'NSW':200, 'NELM':200, 'LVHAR':'.TRUE.'},
	    		popkey=['LCALCEPS', 'ISTART', 'ICHARG', 'KPAR', 'NELMIN', 'ISYM','MAGMOM'])

    def defectposcar(self, defects, neighbordistance,remove,savefilname='POSCAR.defect.vasp',modelcwd=True):
        ...
        change_files=change_input_files(self.folder_to) # will add selective_dynamics for defect POSCAR by default
        change_files.poscar_change(defects, neighbordistance, poscar=self.from_poscar, 
				remove=remove, savefilname=savefilname, modelcwd=modelcwd )
        os.chdir(self.folder_to)
        os.system('cp %s POSCAR' % (savefilname))
        os.system('chmod u+x POSCAR')
        os.chdir(self.path)
