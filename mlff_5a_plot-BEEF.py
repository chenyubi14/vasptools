...
y=[]
if len(sys.argv) < 2:
    print('Error! Enter /path/to/BEEF.dat (can be multiple files)')
    sys.exit()
last_x = 0
for fil in sys.argv[1:]:
    print(fil)
    x_i, y_i = np.loadtxt(fil, usecols=[1,3], unpack=True)
    x_i = x_i + last_x
    x.append(list(x_i))
    y.append(list(y_i))
    last_x = x_i[-1]
#x, y=np.loadtxt("BEEF.dat", usecols=[1,3], unpack=True)
x=[item for sublist in x for item in sublist]
y=[item for sublist in y for item in sublist]

###############################################################################
plt.plot(x, y)
title='Bayesian error estimate of force'
xlabel='Time Steps [ps]'
ylabel='Bayesian error [eV/A]'
plt.title(title)
plt.xlabel(xlabel)
plt.ylabel(ylabel)
###############################################################################
