...
function calc_freysoldt_correction(){
    ...
    # don't update DEFECT file, use the current --center for freysoldt
    sxcharge=` expr 0 - $charge ` # freysoldt charge is opposite to the defect charge
    freydir='freysoldt_correction_ref-'${ref}'.test' # freysoldt directory name
    ...
    if [ -e 'LOCPOT' ]; then # file LOCPOT exists
        ...
        cp DEFECT.test $freydir # copy DEFECT is unnecessary
    else
    ...
    defectcenter=$(cat DEFECT.test | sed -n '/CENTER/{s/CENTER=//;s/#.*//;p}' ) # remove keyword and remove comments #.*
    ...
    paraC=$(python ${SCRIPT}/draw_freysoldt_potential_alignment.py $defecttype $charge $defectcenter | sed -n '/paraC/{s/paraC=//;p}')
    ...
    echo ${freycorr}=${freycorrenergy} >> DEFECT.test
...
# initialization
if [ -z $1 ] || [ -z $2 ] || [ -z $3 ]; then
    echo -e '\n defect charge, defect folder, and reference folder are not found! Enter the charge ($1) defect folder ($2) and perfect folder ($3) or elec ($3) for electronic.\n Folders should start with /'
    echo -e '${WORK1}/wurtzite_01_super_perfect/perfect_run_aexx0.405\n${WORK1}/wurtzite_01_super_perfect/perfect_run_aexx0.405_specialKPOINT\n${WORK1}/wurtzite_09_super_perfect_symm/perfect_run_aexx0.405'
...
defecttype=$( cat DEFECT.test | sed -n '/DEFECTTYPE/p' | sed -e 's/mathit//g' -e 's/mathrm//g' -e 's/DEFECTTYPE=//g' -e 's/#.*//g' | tr -d '{}\\$' )
if [ -z $defecttype ]; then
    echo 'Error! DEFECTTYPE does not exist in DEFECT.test file. Modify the DEFECT.test files!'
    exit 0
fi
