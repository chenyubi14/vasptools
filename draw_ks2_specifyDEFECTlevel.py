...
if len(sys.argv) < 2:
        ...
        print('\n Remember to put UPDEFECTLEVEL,DOWNDEFECTLEVEL, and DEFECTTYPE(title) in file DEFECT')
        sys.exit()
f1=Path(sys.argv[1])
f2=Path(pwd)
...
def plot_ks(ax,ksupstates,ksdownstates,maxx,maxy, defectenergyVBM,tol=1.5, initial_mode=2):
        '''
        ...
        '''
        plot_panel_ks(ax, ksupstates, maxx, maxy, defectenergyVBM, tol=tol, initial_mode=initial_mode, updown=1)
        plot_panel_ks(ax, ksdownstates, maxx, maxy, defectenergyVBM, tol=tol, initial_mode=initial_mode, updown=0)
...
selected_up = arraystring2list(mydict['UPDEFECTLEVEL'])
selected_down = arraystring2list(mydict['DOWNDEFECTLEVEL'])
